...
public:
    ...
    char* NewRow() {
        if (batchRemaining_ <= 0) {
            ...
            rowBlocks_.push_back(batchPtr_);
    ...
    int64_t AggI64Checksum(int32_t aggOffset) const {
        int64_t checksum = 0;
        int64_t remaining = numRows_;
        for (auto* block : rowBlocks_) {
            if (remaining <= 0) break;
            int64_t rowsInBlock = std::min<int64_t>(remaining, kBatchSize);
            for (int64_t rowIdx = 0; rowIdx < rowsInBlock; rowIdx++) {
                checksum += ReadValue<int64_t>(block + rowIdx * fixedRowSize_, aggOffset);
            }
