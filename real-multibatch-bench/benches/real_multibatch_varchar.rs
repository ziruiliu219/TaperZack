    ...
    for (row_idx, &h) in hashes.iter().enumerate() {
        ...
        match entry {
            ...
            RawEntryMut::Occupied(e) => {
                sums[*e.get() as usize] += batch.values[row_idx];
            }
        }
    }
...
fn aggregate_partial_generic(rows: &PartialRows) -> PartialRows {
    ...
    let mut hashes = Vec::with_capacity(len);
    for row_idx in 0..len {
        hashes.push(hash_partial_row(rows, row_idx));
    }
    ...
    for (row_idx, &h) in hashes.iter().enumerate() {
        ...
        match entry {
            ...
            RawEntryMut::Occupied(e) => {
                sums[*e.get() as usize] += rows.sums[row_idx];
            }
        }
    }
...
fn symbolize_cols(str_cols: &[Vec<Vec<u8>>]) -> Vec<Vec<u32>> {
    ...
    for col in str_cols {
        let mut map = HashMap::<&[u8], u32>::with_capacity_and_hasher(
            len.min(1024).max(1),
            Default::default(),
        );
        let mut next = 0u32;
...
fn aggregate_packed_two_symbols(symbols: &[Vec<u32>], values: &[i64]) -> (Vec<usize>, Vec<i64>) {
    let len = values.len();
    let mut table = HashMap::<u64, u32>::with_capacity_and_hasher(
        len.min(1024).max(1),
        Default::default(),
    );
    let mut reps = Vec::<usize>::new();
...
fn run_taper_persistent(workload: &Workload) -> usize {
    ...
    for _ in 0..workload.num_str_cols {
        descs.push(ColumnDesc::Varchar);
    }
    for _ in 0..workload.num_int_cols {
        descs.push(ColumnDesc::Int64);
    }
